...
resource "aws_lb_target_group" "targetgroup" {
  ...
  tags = {
    Name = "TargetGroup-private"
  }
}
resource "aws_lb_target_group_attachment" "private" {
  count             = length(var.ec2_instance_ids-private)  # Loop over the list of EC2 instance IDs
  target_group_arn  = aws_lb_target_group.targetgroup.arn
  target_id         = var.ec2_instance_ids-private[count.index]  # Directly use the EC2 instance ID from the list
  port              = 80
}
